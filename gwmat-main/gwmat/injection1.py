...
class CBCInjection1(PointLens, GWUtils, GeneralUtils):
    ...
    params : dict
        ...
        **EMRI Parameters (for EMRI waveform generation using FEW package):**

            * emri_M : ({1e6, float}), optional
                Primary massive black hole mass in solar masses. Default is 1e6.

            * emri_mu : ({50.0, float}), optional
                Secondary compact object mass in solar masses. Default is 50.0.

            * emri_p0 : ({10.0, float}), optional
                Initial semi-latus rectum (10 <= p0 <= 16 + e0). Default is 10.0.

            * emri_e0 : ({0.4, float}), optional
                Initial eccentricity (0.0 <= e0 <= 0.7). Default is 0.4.

            * emri_theta : ({pi/4, float}), optional
                Polar viewing angle in radians. Default is pi/4.

            * emri_phi : ({pi/3, float}), optional
                Azimuthal viewing angle in radians. Default is pi/3.

            * emri_dist : ({1.0, float}), optional
                Luminosity distance in Gpc. Default is 1.0.

            * emri_dt : ({10.0, float}), optional
                Time step in seconds (inverse of sampling frequency). Default is 10.0.

            * emri_T : ({2.0, float}), optional
                Total observation time in years. Default is 2.0.

            * emri_Phi_phi0 : ({0.0, float}), optional
                Initial phase for Phi_phi. Default is 0.0.

            * emri_Phi_r0 : ({0.0, float}), optional
                Initial phase for Phi_r. Default is 0.0.

            * emri_mode_selection : (list or None), optional
                List of (l, m, n) mode tuples to include, or None for automatic
                mode filtering. Default is None.

            * emri_use_gpu : ({False, bool}), optional
                Whether to use GPU acceleration. Default is False.

            * emri_inspiral_kwargs : (dict), optional
                Kwargs for the inspiral generator. Default includes DENSE_STEPPING=0
                and max_init_len=1000.

            * emri_amplitude_kwargs : (dict), optional
                Kwargs for the amplitude generator. Default includes max_init_len=1000.

            * emri_sum_kwargs : (dict), optional
                Kwargs for the summation generator. Default includes pad_output=False.

            * emri_Ylm_kwargs : (dict), optional
                Kwargs for the Ylm generator. Default includes assume_positive_m=False.
    """
    ...
    def _set_emri_defaults(self, **params):
        """
        Set default values for EMRI waveform parameters if not provided.

        This method applies default values for all EMRI-specific parameters
        used by the FastSchwarzschildEccentricFlux waveform generator from
        the FEW package.

        Parameters
        ----------
        **params : dict
            Dictionary of parameters that may contain EMRI-specific keys.

        Returns
        -------
        dict
            Updated parameters dictionary with EMRI defaults applied.
        """
        emri_defaults = {
            "emri_M": 1e6,  # Primary MBH mass (solar masses)
            "emri_mu": 50.0,  # Secondary compact object mass (solar masses)
            "emri_p0": 10.0,  # Initial semi-latus rectum
            "emri_e0": 0.4,  # Initial eccentricity (0 <= e0 <= 0.7)
            "emri_theta": np.pi / 4.0,  # Polar viewing angle
            "emri_phi": np.pi / 3.0,  # Azimuthal viewing angle
            "emri_dist": 1.0,  # Luminosity distance (Gpc)
            "emri_dt": 10.0,  # Time step (seconds)
            "emri_T": 2.0,  # Observation duration (years)
            "emri_Phi_phi0": 0.0,  # Initial phi phase
            "emri_Phi_r0": 0.0,  # Initial r phase
            "emri_mode_selection": None,  # None = automatic mode filtering
            "emri_use_gpu": False,  # GPU acceleration flag
            # Generator kwargs
            "emri_inspiral_kwargs": {
                "DENSE_STEPPING": 0,
                "max_init_len": int(1e3),
            },
            "emri_amplitude_kwargs": {
                "max_init_len": int(1e3),
            },
            "emri_sum_kwargs": {
                "pad_output": False,
        ...
        for key, default in emri_defaults.items():
 ...
 has been assigned by default."
    ...
    def generate_unlensed_emri_waveforms(self, **params):
        """
        Generate EMRI waveforms in time-domain and frequency-domain polarization
        modes using the FastSchwarzschildEccentricFlux model from the FEW package.

        Parameters
        ----------
        **params : dict
            Dictionary of EMRI parameters. All parameters are prefixed with 'emri_'.
            See class docstring for full list of EMRI parameters.

            Key parameters:
                - emri_M: Primary MBH mass (solar masses). Default: 1e6.
                - emri_mu: Secondary compact object mass (solar masses). Default: 50.
                - emri_p0: Initial semi-latus rectum. Default: 10.0.
                - emri_e0: Initial eccentricity (0 <= e0 <= 0.7). Default: 0.4.
                - emri_theta: Polar viewing angle (radians). Default: pi/4.
                - emri_phi: Azimuthal viewing angle (radians). Default: pi/3.
                - emri_dist: Luminosity distance (Gpc). Default: 1.0.
                - emri_dt: Time step in seconds. Default: 10.0.
                - emri_T: Observation duration in years. Default: 2.0.
                - emri_mode_selection: List of (l, m, n) tuples or None. Default: None.
                - emri_use_gpu: Whether to use GPU acceleration. Default: False.

        Returns
        -------
        dict
            Dictionary containing:
                - frequency_array: Array of frequencies for FD waveforms.
                - hp_TD_Unlensed: Time-domain plus polarization (PyCBC TimeSeries).
                - hc_TD_Unlensed: Time-domain cross polarization (PyCBC TimeSeries).
                - hp_FD_Unlensed: Frequency-domain plus polarization (PyCBC FrequencySeries).
                - hc_FD_Unlensed: Frequency-domain cross polarization (PyCBC FrequencySeries).
        """
        # Apply EMRI defaults for any missing parameters
        params = self._set_emri_defaults(**params)

        # Extract EMRI parameters from params dict
        M = params["emri_M"]
        mu = params["emri_mu"]
        p0 = params["emri_p0"]
        e0 = params["emri_e0"]
        theta = params["emri_theta"]
        phi = params["emri_phi"]
        dist = params["emri_dist"]
        dt = params["emri_dt"]
        T = params["emri_T"]
        Phi_phi0 = params["emri_Phi_phi0"]
        Phi_r0 = params["emri_Phi_r0"]
        mode_selection = params["emri_mode_selection"]
        use_gpu = params["emri_use_gpu"]

        # Build generator kwargs from params, adding use_gpu where needed
        inspiral_kwargs = params["emri_inspiral_kwargs"].copy()
        amplitude_kwargs = {
            **params["emri_amplitude_kwargs"],
            "use_gpu": use_gpu,
        }
        sum_kwargs = {
            **params["emri_sum_kwargs"],
            "use_gpu": use_gpu,
        }
        Ylm_kwargs = params["emri_Ylm_kwargs"].copy()

        # Instantiate the FastSchwarzschildEccentricFlux waveform generator
        ...
        # Generate the waveform (complex: hp - i*hc)
        wave = few_gen(
            M, mu, p0, e0, theta, phi,
            dist=dist,
            dt=dt,
            T=T,
            Phi_phi0=Phi_phi0,
        ...
        # Convert to PyCBC TimeSeries (real part = hp, imag part = hc)
        td_hp = TimeSeries(
            initial_array=np.real(wave),
            delta_t=dt,
            dtype=np.float64
        )
        td_hc = TimeSeries(
            initial_array=-np.imag(wave),  # FEW returns hp - i*hc, so hc = -imag
            delta_t=dt,
            dtype=np.float64
        )
    ...
    def generate_unlensed_emri_waveforms_FD(self, **params):
        """
        Generate EMRI waveforms with explicit FFT-based frequency-domain output.

        This function generates EMRI waveforms using the FastSchwarzschildEccentricFlux
        model and performs an explicit FFT to obtain the frequency-domain representation.
        This provides more control over the FFT process compared to using PyCBC's
        ...
        **params : dict
        ...
        dict
            Dictionary containing:
                - frequency_array: Array of FFT frequencies.
                - hp_TD_Unlensed: Time-domain plus polarization (PyCBC TimeSeries).
                - hc_TD_Unlensed: Time-domain cross polarization (PyCBC TimeSeries).
                - hp_FD_Unlensed: Frequency-domain plus polarization (PyCBC FrequencySeries).
                - hc_FD_Unlensed: Frequency-domain cross polarization (PyCBC FrequencySeries).
        ...
        Notes
        -----
        This function uses numpy's FFT directly rather than PyCBC's to_frequencyseries()
        method, which may be useful for certain analysis pipelines.
        """
        # Apply EMRI defaults for any missing parameters
        params = self._set_emri_defaults(**params)
        ...
        # Perform explicit FFT
        fft_wave = np.fft.fft(wave) * dt
        freq_fft = np.fft.fftfreq(len(wave), dt)
        ...
        # Create frequency-domain series from FFT
        # Note: fft_wave is complex, real part corresponds to hp, imag to -hc
